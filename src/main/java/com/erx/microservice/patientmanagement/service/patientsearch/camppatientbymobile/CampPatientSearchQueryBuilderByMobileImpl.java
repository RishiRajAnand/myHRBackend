...
@Service("campPatientSearchQueryBuilderByMobile")
public class CampPatientSearchQueryBuilderByMobileImpl implements CampPatientSearchQueryBuilderByMobile {

    private final Logger log = LoggerFactory.getLogger(CampPatientSearchQueryBuilderByMobileImpl.class);
    ...
    public List<CampRegistrationDTO> getCampPatientSearchResults(PatientSearchCriteria patientSearchCriteria) {
        ...
        try {
            if (patientSearchCriteria.getClinicId() != null & patientSearchCriteria.getSearchType() != null & patientSearchCriteria.getSearchValue() != null) {
                whereClause = " where p.recordStatus= 1 and p.clinic.id= " + patientSearchCriteria.getClinicId() + " and p.mobileNumber = '" + patientSearchCriteria.getSearchValue() + "' ORDER BY pcr.createdOn DESC";
