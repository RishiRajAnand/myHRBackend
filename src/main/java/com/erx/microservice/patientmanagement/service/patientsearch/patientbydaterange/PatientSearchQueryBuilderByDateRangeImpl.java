package com.erx.microservice.patientmanagement.service.patientsearch.patientbydaterange;
...
@Service("patientSearchQueryBuilderByDateRange")
public class PatientSearchQueryBuilderByDateRangeImpl implements PatientSearchQueryBuilderByDateRange {

    private final Logger log = LoggerFactory.getLogger(PatientSearchQueryBuilderByDateRangeImpl.class);
    ...
    public List<PatientSearchDTO> getPatientSearchResults(PatientSearchCriteria patientSearchCriteria) {
        ...
        try {
            ...
            if (patientSearchCriteria.getClinicId() != null & patientSearchCriteria.getSearchType() != null & patientSearchCriteria.getSearchValue() != null) {
                ...
                LocalDateTime startDate = LocalDateTime.parse(patientSearchCriteria.getStartDate());
                LocalDateTime endDate = LocalDateTime.parse(patientSearchCriteria.getEndDate());
                whereClause = " where p.recordStatus= 1 and p.registered = TRUE and NOT p.patientIdExternal=null and p.clinic.id= " + patientSearchCriteria.getClinicId() + " and p.patientRegisteredDate BETWEEN '" + startDate + "' AND '" + endDate + "'";
