...
@Service("campPatientSearchQueryBuilderByMRN")
public class CampPatientSearchQueryBuilderByMRNImpl implements CampPatientSearchQueryBuilderByMRN {

    private final Logger log = LoggerFactory.getLogger(CampPatientSearchQueryBuilderByMRNImpl.class);
    ...
    public List<CampRegistrationDTO> getCampPatientSearchResults(PatientSearchCriteria patientSearchCriteria) {
        ...
        try {
            if (patientSearchCriteria.getClinicId() != null & patientSearchCriteria.getSearchType() != null & patientSearchCriteria.getSearchValue() != null) {
                whereClause = " where p.recordStatus= 1 and p.clinic.id= " + patientSearchCriteria.getClinicId() + " and p.patientIdExternal = '" + patientSearchCriteria.getSearchValue() + "' ORDER BY pcr.createdOn DESC";
