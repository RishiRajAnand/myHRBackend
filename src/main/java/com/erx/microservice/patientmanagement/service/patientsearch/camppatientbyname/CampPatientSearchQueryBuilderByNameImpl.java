...
@Service("campPatientSearchQueryBuilderByName")
public class CampPatientSearchQueryBuilderByNameImpl implements CampPatientSearchQueryBuilderByName {

    private final Logger log = LoggerFactory.getLogger(CampPatientSearchQueryBuilderByNameImpl.class);
        ...
        try {
            if (patientSearchCriteria.getClinicId() != null & patientSearchCriteria.getSearchType() != null & patientSearchCriteria.getSearchValue() != null) {
                whereClause = " where p.recordStatus= 1 and p.clinic.id= " + patientSearchCriteria.getClinicId() +
                        " and p.patientName like '%" + patientSearchCriteria.getSearchValue() + "%' ORDER BY pcr.createdOn DESC";
