...
@Service("patientCaseCountSearchQueryBuilderById")
public class PatientCaseCountSearchQueryBuilderByIdImpl implements PatientCaseCountSearchQueryBuilderById {
    private final Logger log = LoggerFactory.getLogger(PatientCaseCountSearchQueryBuilderByIdImpl.class);
    ...
    public Page<PatientCaseCountSearchDTO> getPatientSearchResults(PatientSearchServiceRequest request) {
        ...
        try {
            ...
            patient = patientRepository.getPatientDetailsByPatientId(request.getPatientSearchCriteria().getClinicId(),request.getPatientSearchCriteria().getSearchValue(),pageable);
            if(patient != null)
           ...
           log.error("failed to retrieve patient and case count based on patient id");
        }
