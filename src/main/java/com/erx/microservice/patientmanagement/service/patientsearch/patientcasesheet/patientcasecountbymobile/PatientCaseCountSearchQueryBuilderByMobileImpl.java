...
@Service("patientCaseCountSearchQueryBuilderByMobile")
public class PatientCaseCountSearchQueryBuilderByMobileImpl implements PatientCaseCountSearchQueryBuilderByMobile {
    private final Logger log = LoggerFactory.getLogger(PatientCaseCountSearchQueryBuilderByMobileImpl.class);
    ...
    public Page<PatientCaseCountSearchDTO> getPatientSearchResults(PatientSearchServiceRequest request) {
        ...
        try {
            ...
            patient = patientRepository.getPatientDetailsByMobileNumber(request.getPatientSearchCriteria().getClinicId(),request.getPatientSearchCriteria().getSearchValue(),pageable);
            if(patient != null)
                log.debug("call to search patient along with case count by patient mobile number");
          ...
          log.error("failed to retrieve patient and case count based on mobile number");
        }
