...
@Service("campPatientSearchQueryBuilderByLocation")
public class CampPatientSearchQueryBuilderByLocationImpl implements CampPatientSearchQueryBuilderByLocation {

    private final Logger log = LoggerFactory.getLogger(CampPatientSearchQueryBuilderByLocationImpl.class);
    ...
    public List<CampRegistrationDTO> getCampPatientSearchResults(PatientSearchCriteria patientSearchCriteria) {
        ...
        try {
            if (patientSearchCriteria.getClinicId() != null & patientSearchCriteria.getSearchType() != null & patientSearchCriteria.getSearchValue() != null) {
                whereClause = " where p.recordStatus= 1 and p.clinic.id= " + patientSearchCriteria.getClinicId() + " and p.clinicLocation.location.name = '" + patientSearchCriteria.getSearchValue() + "' ORDER BY pcr.createdOn DESC";
